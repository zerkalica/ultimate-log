var ul = require('../ul');
var pathway = require('../utils/pathway');
...
function StdLogger(config) {
	...
	function configToTransports(item) {
		...
		if (item.formatter) {
			var Formatter = typeof item.formatter === 'object' ? item.formatter : (pathway(ul.Formatters, item.formatter) || ul.Formatters.TokenFormatter);
			item.formater = new Formatter(item);
		}
