var proto,
	...
	getSerializedData = require('./utils/get-serialized-data');
...
proto.log = function Logger_log(logObject, type) {
	...
	for (var prop in logObject) {
		simpleLogObject[prop] = getSerializedData(logObject[prop], this._timeFormat);
	}

	if (!this._firstSessionId) {
		this._firstSessionId = 'fb.' + unique();
		simpleLogObject.direct = true;
	}
	simpleLogObject.id   = simpleLogObject.id || this._firstSessionId;
	simpleLogObject.type = simpleLogObject.type || 'info';

	this._transports.log(simpleLogObject);
};
